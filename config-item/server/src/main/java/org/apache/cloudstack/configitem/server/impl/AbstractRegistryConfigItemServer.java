...
public abstract class AbstractRegistryConfigItemServer implements Registry<ConfigItem>, ConfigItemServer {
    ...
    @Override
    public boolean register(ConfigItem type) {
        if (items.containsKey(type.getName()))
            return false;

        items.put(type.getName(), type);

        return true;
    }

    @Override
    public void unregister(ConfigItem type) {
        items.remove(type.getName());
    }

    @Override
    public List<ConfigItem> getRegistered() {
        return new ArrayList<ConfigItem>(items.values());
    }
